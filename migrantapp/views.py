...
db=pymysql.connect(host='localhost',user='root',password='',database='dbmigrants')
c=db.cursor()
...
def adminaddpolice(request):
    ...
    if(request.POST):
        ...
        if(i[0]>0):
            ...
            s="insert into tblpolicestation (pName,pStationCode,pAddress,pCircle,pContact,pEmail) values('"+name+"','"+stationcode+"','"+address+"','"+circle+"','"+phone+"','"+email+"')"
            try:
                c.execute(s)
                db.commit()
            except:
                ...
                s="insert into tbllogin (username,password,utype,status) values('"+email+"','"+email+"','police','1')"
                try:
                    c.execute(s)
                    db.commit()
                except:
...
def adminaddlabor(request):
    ...
    if(request.POST):
        ...
        if(i[0]>0):
            ...
            s="insert into tbllabour (pEmail,labName,labAddress,labPlace,labIdMark1,labIdMark2,labPhone,labAadhar,labPhoto,labHeight,labWeight,labColor,labDisease,status,signature,aadharfile) values('"+station+"','"+name+"','"+address+"','"+place+"','"+id1+"','"+id2+"','"+phone+"','"+aadhar+"','"+uploaded_file_url+"','"+height+"','"+weight+"','"+color+"','"+disease+"','registered','"+uploaded_file_url1+"','"+uploaded_file_url2+"')"
            try:
                c.execute(s)
                db.commit()
            except:
...
def adminviewlabordetails(request):
    ...
    s="select tbllabour.*,tblpolicestation.pName from tbllabour,tblpolicestation where tbllabour.labId ='"+lid+"' and tbllabour.pEmail=tblpolicestation.pEmail"
    c.execute(s)
    data=c.fetchall()
...
def admingeneratecard(request):
    ...
    try:
        ...
        s="insert into tblglobalid (labId,dateIssue,idStatus) values('"+str(lid)+"',(select sysdate()),'active')"
        try:
            c.execute(s)
            db.commit()
        except:
...
def jobcard(request):
    ...
    s="select tbllabour.*,tblglobalid.globalId,tblcontractor.conName,tblcontractor.conPhone from tbllabour,tblcontractor,tblglobalid where tbllabour.labId=tblglobalid.labId and tblcontractor.conEmail=tbllabour.conEmail and tblglobalid.globalId='"+str(gid)+"'"
    c.execute(s)
    i=c.fetchall()
...
def policehome(request):
    ...
    if(request.POST):
        ...
        s="update tblpolicestation set pName='"+name+"',pStationCode='"+stationcode+"',pAddress='"+address+"',pCircle='"+circle+"',pContact='"+phone+"' where pEmail='"+email+"'"
        try:
            c.execute(s)
            db.commit()
        except:
...
def policeviewreglabors(request):
    ...
    """
    email=request.session["email"]
    s="select * from tbllabour where labId not in(select labId from tblglobalid) and pEmail='"+email+"' and status='registered'"
    c.execute(s)
    data=c.fetchall()
...
def policeviewlabordetails(request):
    ...
    """
    lid=request.GET.get("id")
    s="select tbllabour.*,tblpolicestation.pName from tbllabour,tblpolicestation where tbllabour.labId ='"+str(lid)+"' and tbllabour.pEmail=tblpolicestation.pEmail"
    c.execute(s)
    data=c.fetchall()
...
def policeviewcomplaint(request):
    ...
    """
    email=request.session["email"]
    s="select tblcomplaint.*,tbluser.* from tblcomplaint,tbluser where tblcomplaint.pEmail='"+email+"' and tblcomplaint.uEmail=tbluser.uEmail"
    c.execute(s)
    data=c.fetchall()
...
def policesearchsimilarlabor(request):
    ...
    w1=int(weight)-10
    w2=int(weight)+10
    s="select tbllabour.*,tblglobalid.globalId,tblcontractor.conName,tblcontractor.conPhone from tbllabour,tblcontractor,tblglobalid where tbllabour.labId=tblglobalid.labId and tblcontractor.conEmail=tbllabour.conEmail and tbllabour.labHeight between '"+str(h1)+"' and '"+str(h2)+"' and tbllabour.labWeight between '"+str(w1)+"' and '"+str(w2)+"'"
    c.execute(s)
    data=c.fetchall()
...
def contractorreg(request):
    ...
    if(request.POST):
        ...
        if(i[0]>0):
            ...
            s="insert into tblcontractor (conName,conAddress,conPhone,conEmail,conLicense,conCertificate) values('"+name+"','"+address+"','"+phone+"','"+email+"','"+licenseno+"','"+uploaded_file_url+"')"
            try:
                c.execute(s)
                db.commit()
            except:
                ...
                s="insert into tbllogin (username,password,utype,status) values('"+email+"','"+pwd+"','contractor','0')"
                try:
                    c.execute(s)
                    db.commit()
                except:
...
def contractorhome(request):
    ...
    if(request.POST):
        ...
        s="update tblcontractor set conName='"+name+"',conLicense='"+clicense+"',conAddress='"+address+"',conPhone='"+phone+"' where conEmail='"+email+"'"
        try:
            c.execute(s)
            db.commit()
        except:
...
def userreg(request):
    ...
    if(request.POST):
        ...
        if(i[0]>0):
            ...
            s="insert into tblUser (uName,uContact,uEmail,uAadhar) values('"+name+"','"+phone+"','"+email+"','"+aadhar+"')"
            try:
                c.execute(s)
                db.commit()
            except:
...
def userhome(request):
    ...
    if(request.POST):
        ...
        email=request.POST["Email"]
        phone=request.POST["Phone"]
        s="update tbluser set uName='"+name+"',uAadhar='"+aadhar+"',uContact='"+phone+"' where uEmail='"+email+"'"
        try:
            c.execute(s)
            db.commit()
        except:
...
def userworkrequest(request):
    ...
    if(request.POST):
        ...
        l1=request.POST["l1"]
        l2=request.POST["l2"]
        s="insert into tbltender(uEmail,reqDescription,reqDate,reqPlace,reqLat,reqLon,reqStatus) values('"+email+"','"+work+"','"+wdate+"','"+place+"','"+l1+"','"+l2+"','requested')"
        try:
            c.execute(s)
            db.commit()
        except:
            ...
            msg="Data updated successfully"
    return render(request,"userrequest.html",{"msg":msg})
######################################################################
#                   VIEW TENDER CALLS
######################################################################
...
def useraddfeedback(request):
    ...
    if(i[0]>0):
        ...
        if(request.POST):
            ...
            s="insert into tblfeedback(workId,fdate,feedback) values('"+wid+"',(select sysdate()),'"+feedback+"')"
            try:
                c.execute(s)
                db.commit()
            except:
